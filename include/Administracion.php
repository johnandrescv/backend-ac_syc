...
class DbHandler {
    ...
    */
    ...
    public function editUsuario($id, $dni, $nombres, $id_tipo, $edad, $correo) {
        if (!$this->isUsuarioExists($dni, $id)) {
            $response = array();
            $stmt = $this->conn->prepare("UPDATE usuarios SET dni = ?, nombres = ?, id_tipo = ?, edad = ?, correo = ? WHERE id_usuario = ?");
            $stmt->bind_param("ssssss", $dni, $nombres, $id_tipo, $edad, $correo, $id);
            $result = $stmt->execute();
            //printf("Error: %s.\n", $stmt->error);
            $stmt->close();
    ...
    */
    ...
    public function gestionSocios($data) {
        ...
        foreach($socios as $socio) {
            ...
            if($existe === false) {
                ...
                $this->activateUser($existe["id_usuario"], $socio["status"], $socio["codigo"], $socio["parentesco"]);
            }
        }
        return true;
    }
    ...
    public function activateUser($id, $status, $codigo, $parentesco) {
        $response = array();
        $stmt = $this->conn->prepare("UPDATE usuarios SET socio_status = ?, codigo = ?, parentesco = ?, estado = 'A' WHERE id_usuario = ?");
        $stmt->bind_param("ssss", $status, $codigo, $parentesco, $id);
        $result = $stmt->execute();
        //printf("Error: %s.\n", $stmt->error);
        $stmt->close();
